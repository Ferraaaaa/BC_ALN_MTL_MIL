...
def softmax_metrics(results, gt_seg_maps, num_classes):
    ...
    total_p = np.zeros((num_classes,), dtype=np.float32)
    total_tp = np.zeros((num_classes,), dtype=np.float32)
    total_fn = np.zeros((num_classes,), dtype=np.float32)
    ...
    return total_p, total_tp, total_fn
...
def sigmoid_confused_matrix(pred_logit, raw_label, num_classes, thresh):
    ...
    class_p = np.zeros((num_classes,), dtype=np.float32)
...
def metrics(results, gt_seg_maps, num_classes,use_sigmoid, ignore_index=None, nan_to_num=None):
    """
    :param results: feature map after sigmoid of softmax
    """
    if not use_sigmoid:
        total_p, total_tp, total_fn = softmax_metrics(
            results, gt_seg_maps, num_classes)
    else:
        total_p, total_tp, total_fn = sigmoid_metrics(
            results, gt_seg_maps, num_classes)
